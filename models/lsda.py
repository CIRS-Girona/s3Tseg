...
class LSDABlock(nn.Module):
    ...
    def __init__(self, dim, patches_resolution, num_heads, group_size=8, lsda_flag=0, mlp_ratio=4.,
        ...
        if self.patches_resolution <= self.group_size:
...
class BasicEncoderLayer(nn.Module):
    """ A basic LSDA layer for one encoder stage.
    Args:
        dim (int): Number of input channels.
...
class BasicDecoderLayer(nn.Module):
    """ A basic LSDA layer for one decoder stage.
    Args:
        dim (int): Number of input channels.
        patches_resolution (int): Input patch resolution.
        depth (int): Number of blocks.
...
class LSDAEncoder(nn.Module):
    ...
    def __init__(self, img_size=256, in_chans=1, num_classes=0, patch_size=4, embed_dim=64,
                 depths=[2,2,6,2], group_size=8, num_heads=[2,4,8,16], mlp_ratio=4., qkv_bias=True,
                 qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, use_ghost_ffn=False,
...
class LSDADecoder(nn.Module):
    ...
    def __init__(self, img_size=256, in_chans=1, num_classes=0, patch_size=4, embed_dim=64,
                 depths=[6, 2, 2], group_size=8, num_heads=[8, 4, 2], mlp_ratio=4.,
...
@encoder_entrypoints.register('lsda')
def build_encoder(config):
    encoder_config = config.MODEL.ENCODER
    return LSDAEncoder(
            config.DATA.IMAGE_SIZE, config.DATA.IN_CHANS, 0,
            patch_size=encoder_config['PATCH_SIZE'],
            group_size=encoder_config['GROUP_SIZE'],
...
@decoder_entrypoints.register('lsda')
